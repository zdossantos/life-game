...
import type { Grid, GameSettings } from '@/types/game-of-life';
    ...
    for (let i = 0; i < settings.value.gridSize; i++) {
        for (let j = 0; j < settings.value.gridSize; j++) {
            ...
            if (grid.value[i][j].alive === 1) {
                newGrid[i][j].alive = (neighbors >= settings.value.neighborThresholds.surviveMin &&
                    neighbors <= settings.value.neighborThresholds.surviveMax) ? 1 : 0;
            } else {
...
const countNeighbors = (row: number, col: number): number => {
    ...
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            ...
            const newRow = row + i;
            const newCol = col + j;
            if (newRow >= 0 && newRow < settings.value.gridSize &&
                newCol >= 0 && newCol < settings.value.gridSize) {
...
watch(() => settings.value.updateSpeed, (newSpeed) => {
    if (isRunning.value && intervalId) {
        clearInterval(intervalId);
