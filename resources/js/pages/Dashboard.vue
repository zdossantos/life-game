    ...
    <DashboardLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-6 p-4 md:p-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold tracking-tight">Bonjour, {{ user.name }} 👋</h2>
                    <p class="mt-1 text-sm text-muted-foreground">Voici un aperçu de vos simulations</p>
                </div>
                <Link
                    :href="route('home')"
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Card>
                    <CardHeader class="flex flex-row items-center justify-between pb-2">
                        <CardTitle class="text-sm font-medium text-muted-foreground">Parties sauvegardées</CardTitle>
                        <Gamepad2 class="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <p class="text-3xl font-bold">{{ totalSaves }}</p>
                        <p class="mt-1 text-xs text-muted-foreground">simulations enregistrées</p>
                    </CardContent>
                </Card>

                <Card>
                    <CardHeader class="flex flex-row items-center justify-between pb-2">
                        <CardTitle class="text-sm font-medium text-muted-foreground">Cycles totaux</CardTitle>
                        <RefreshCw class="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <p class="text-3xl font-bold">{{ totalCycles.toLocaleString('fr-FR') }}</p>
                        <p class="mt-1 text-xs text-muted-foreground">générations simulées</p>
                    </CardContent>
                </Card>

                <Card class="sm:col-span-2 lg:col-span-1">
                    <CardHeader class="flex flex-row items-center justify-between pb-2">
                        <CardTitle class="text-sm font-medium text-muted-foreground">Tailles de grille</CardTitle>
                        <Grid2X2 class="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <p class="text-3xl font-bold">{{ uniqueGridSizes }}</p>
                        <p class="mt-1 text-xs text-muted-foreground">tailles différentes utilisées</p>
                    </CardContent>
                </Card>
            </div>
            ...
            <Card class="flex-1">
                <CardHeader>
                    <CardTitle class="text-base">Dernières simulations</CardTitle>
                </CardHeader>
                <CardContent class="p-0">
                    ...
                    <div v-if="!props.saves || props.saves.length === 0" class="flex flex-col items-center gap-3 px-6 py-12 text-center">
                        ...
                        <div>
                            <p class="text-sm font-medium">Aucune simulation trouvée</p>
                            <p class="mt-1 text-xs text-muted-foreground">Démarrez une nouvelle partie pour voir vos sauvegardes ici.</p>
                        </div>
                        <Link
                            :href="route('home')"
                    ...
                    <template v-else>
                        ...
                        >
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-medium">Grille {{ save.grid_size }}×{{ save.grid_size }}</p>
                                <p class="mt-0.5 text-xs text-muted-foreground">
                                    {{ save.cycle_count }} cycle{{ save.cycle_count !== 1 ? 's' : '' }}
                                </p>
                            </div>
